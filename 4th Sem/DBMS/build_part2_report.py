...
def build_items() -> list[dict]:
    ...
    # LAB 4 - GROUP A
    q4a = [
        ("Project Category Names", "Project CategoryName from categories.", "SELECT CategoryName FROM categories;", None, None),
        ("Project Customer Countries", "Project Country from customers.", "SELECT Country FROM customers;", None, None),
        ("Project City and Country", "Project City, Country from customers.", "SELECT City, Country FROM customers;", None, None),
        ...
        ("German Customer IDs", "SELECT CustomerID from customers who are from Germany.", "SELECT CustomerID FROM customers WHERE Country = 'Germany';", None, None),
        ("Germany or Spain Customers", "SELECT CustomerID, CustomerName from customers who are from Germany or Spain.", "SELECT CustomerID, CustomerName FROM customers WHERE Country IN ('Germany', 'Spain');", None, None),
        ...
        ("Customer City by Name Initial", "SELECT all from customers who are from a city whose name starts with the first letter of your name.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE City LIKE '{{{{FIRST_LETTER}}}}%';", None, "customer_city_initial"),
        ("Exclude Canada and Belgium", "SELECT all from customers who are not from Canada and Belgium.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE Country NOT IN ('Canada', 'Belgium');", None, None),
        ("Customers from G7 Countries", "SELECT all from customers who are from G7 Countries (USA, UK, Canada, France, Germany, Italy, Japan).", f"SELECT {CUSTOMER_COLS} FROM customers WHERE Country IN ('USA', 'UK', 'Canada', 'France', 'Germany', 'Italy', 'Japan');", None, None),
        ("Postal Code Contains 31", "SELECT all from customers who have PostalCode containing 31.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE PostalCode LIKE '%31%';", None, None),
        ("Postal Code 00 in Rome or Paris", "SELECT all from customers having PostalCode containing 00 and from Rome or Paris.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE PostalCode LIKE '%00%' AND City IN ('Rome', 'Paris');", None, None),
        ("Customers Not from Salzburg", "SELECT all from customers who are not from Salzburg.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE City <> 'Salzburg';", None, None),
        ("Employees Born in July", "SELECT all employees who are born in July.", f"SELECT {EMPLOYEE_COLS} FROM employees WHERE MONTH(BirthDate) = 7;", None, None),
        ("Employees with BA Degree", "SELECT all employees having BA degree in Notes.", f"SELECT {EMPLOYEE_COLS} FROM employees WHERE Notes LIKE '%BA%';", None, None),
        ("Suppliers from USA and Japan", "SELECT all suppliers from USA and Japan.", f"SELECT {SUPPLIER_COLS} FROM suppliers WHERE Country IN ('USA', 'Japan');", None, None),
        ("Supplier Count by Country", "Find name and count of suppliers from different countries.", "SELECT Country, COUNT(*) AS SupplierCount FROM suppliers GROUP BY Country;", None, None),
        ("Customer Count by City", "Find name and count of customers from different cities.", "SELECT City, COUNT(*) AS CustomerCount FROM customers GROUP BY City;", None, None),
        ("Orders by Employee", "Find number of orders made by employee along with EmployeeID.", "SELECT EmployeeID, COUNT(*) AS OrderCount FROM orders GROUP BY EmployeeID;", None, None),
        ("Orders by Customer", "Find number of orders made by customers along with CustomerID.", "SELECT CustomerID, COUNT(*) AS OrderCount FROM orders GROUP BY CustomerID;", None, None),
        ("Orders Shipped by Shipper 1 or 3", "Find all orders that are shipped by shippers having 1 or 3 ShipperID.", f"SELECT {ORDER_COLS} FROM orders WHERE ShipperID IN (1, 3);", None, None),
        ("Products in Category 1", "SELECT all product having CategoryID 1.", f"SELECT {PRODUCT_COLS} FROM products WHERE CategoryID = 1;", None, None),
        ("Product Name by Name Initial", "SELECT all products having ProductName that starts with the first letter of your name.", f"SELECT {PRODUCT_COLS} FROM products WHERE ProductName LIKE '{{{{FIRST_LETTER}}}}%';", None, "product_name_initial"),
    ]
    ...
    q4b = [
        ...
        ("Spanish Customer IDs", "SELECT CustomerID from customers who are from Spain.", "SELECT CustomerID FROM customers WHERE Country = 'Spain';", None, None),
        ("Mexico or Italy Customers", "SELECT CustomerID, CustomerName from customers who are from Mexico or Italy.", "SELECT CustomerID, CustomerName FROM customers WHERE Country IN ('Mexico', 'Italy');", None, None),
        ...
        ("Customer City by Name Initial", "SELECT all from customers who are from a city whose name starts with the first letter of your name.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE City LIKE '{{{{FIRST_LETTER}}}}%';", None, "customer_city_initial"),
        ("Exclude Venezuela and Argentina", "SELECT all from customers who are not from Venezuela or Argentina.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE Country NOT IN ('Venezuela', 'Argentina');", None, None),
        ("Customers from G7 Countries", "SELECT all from customers who are from G7 Countries (USA, UK, Canada, France, Germany, Italy, Japan).", f"SELECT {CUSTOMER_COLS} FROM customers WHERE Country IN ('USA', 'UK', 'Canada', 'France', 'Germany', 'Italy', 'Japan');", None, None),
        ("Postal Code Contains 50", "SELECT all from customers who have PostalCode containing 50.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE PostalCode LIKE '%50%';", None, None),
        ("Postal Code 31 in Mexico City or Toulouse", "SELECT all from customers having PostalCode containing 31 and from Mexico City or Toulouse.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE PostalCode LIKE '%31%' AND City IN ('Mexico City', 'Toulouse');", None, None),
        ("Customers Not from Elgin", "SELECT all from customers who are not from Elgin.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE City <> 'Elgin';", None, None),
        ("Employees Born in July", "SELECT all employees who are born in July.", f"SELECT {EMPLOYEE_COLS} FROM employees WHERE MONTH(BirthDate) = 7;", None, None),
        ("Employees with BA Degree", "SELECT all employees having BA degree in Notes.", f"SELECT {EMPLOYEE_COLS} FROM employees WHERE Notes LIKE '%BA%';", None, None),
        ("Suppliers from Australia and Sweden", "SELECT all suppliers from Australia and Sweden.", f"SELECT {SUPPLIER_COLS} FROM suppliers WHERE Country IN ('Australia', 'Sweden');", None, None),
        ("Supplier Count by Country", "Find name and count of suppliers from different countries.", "SELECT Country, COUNT(*) AS SupplierCount FROM suppliers GROUP BY Country;", None, None),
        ("Customer Count by City", "Find name and count of customers from different cities.", "SELECT City, COUNT(*) AS CustomerCount FROM customers GROUP BY City;", None, None),
        ("Orders by Employee", "Find number of orders made by employee along with EmployeeID.", "SELECT EmployeeID, COUNT(*) AS OrderCount FROM orders GROUP BY EmployeeID;", None, None),
        ("Orders by Customer", "Find number of orders made by customers along with CustomerID.", "SELECT CustomerID, COUNT(*) AS OrderCount FROM orders GROUP BY CustomerID;", None, None),
        ("Orders Shipped by Shipper 2 or 4", "Find all orders that are shipped by shippers having 2 or 4 ShipperID.", f"SELECT {ORDER_COLS} FROM orders WHERE ShipperID IN (2, 4);", None, None),
        ("Products in Category 5", "SELECT all product having CategoryID 5.", f"SELECT {PRODUCT_COLS} FROM products WHERE CategoryID = 5;", None, None),
        ("Product Name by Name Initial", "SELECT all products having ProductName that starts with the first letter of your name.", f"SELECT {PRODUCT_COLS} FROM products WHERE ProductName LIKE '{{{{FIRST_LETTER}}}}%';", None, "product_name_initial"),
    ]
...
""".strip()
    ...
    q5a = [
        ...
        ("Add Three Suppliers", "Add 3 new suppliers in suppliers table.", "INSERT INTO suppliers (SupplierName, ContactName, Address, City, PostalCode, Country) VALUES ('Himalayan Traders', 'Aarav Sharma', 'Thamel Road', 'Kathmandu', '44600', 'Nepal'), ('Ganga Foods', 'Priya Singh', 'MG Road', 'Delhi', '110001', 'India'), ('Bengal Supplies', 'Rahim Khan', 'Gulshan Avenue', 'Dhaka', '1212', 'Bangladesh');", "SELECT * FROM suppliers WHERE SupplierName IN ('Himalayan Traders', 'Ganga Foods', 'Bengal Supplies');"),
        ("Add Five Customers", "Add 5 new customers in customers table.", "INSERT INTO customers (CustomerName, ContactName, Address, City, PostalCode, Country) VALUES ('Everest Mart', 'Suman Rai', 'Lazimpat', 'Kathmandu', '44600', 'Nepal'), ('Lotus Store', 'Anita Shah', 'Connaught Place', 'Delhi', '110001', 'India'), ('Padma Foods', 'Farhan Ali', 'Dhanmondi', 'Dhaka', '1209', 'Bangladesh'), ('Dragon Market', 'Tashi Dorji', 'Norzin Lam', 'Thimphu', '11001', 'Bhutan'), ('Island Grocers', 'Amina Latheef', 'Majeedhee Magu', 'Malé', '20026', 'Maldives');", "SELECT * FROM customers WHERE CustomerName IN ('Everest Mart', 'Lotus Store', 'Padma Foods', 'Dragon Market', 'Island Grocers');"),
        ("Add Four Products", "Add 4 products in product table.", "INSERT INTO products (ProductName, SupplierID, CategoryID, Unit, Price) VALUES ('Nepal Tea', 1, 1, '20 tea bags', 12.50), ('Tokyo Noodles', 2, 5, '10 packets', 8.75), ('Cajun Spice Mix', 3, 3, '6 jars', 16.00), ('Nordic Cheese', 5, 4, '12 packs', 24.50);", "SELECT * FROM products WHERE ProductName IN ('Nepal Tea', 'Tokyo Noodles', 'Cajun Spice Mix', 'Nordic Cheese');"),
        ("Add Three Employees", "Add 3 new employees.", "INSERT INTO employees (LastName, FirstName, BirthDate, Notes) VALUES ('Shrestha', 'Anil', '1995-01-15', 'BSc in Computer Science.'), ('Karki', 'Mina', '1994-05-21', 'MBA in Operations.'), ('Gurung', 'Rita', '1996-11-10', 'BA in Management.');", "SELECT * FROM employees WHERE LastName IN ('Shrestha', 'Karki', 'Gurung');"),
        ("Brazil to Singapore", "Update all customers from Brazil to Singapore.", "UPDATE customers SET Country = 'Singapore' WHERE Country = 'Brazil';", "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE Country = 'Singapore';"),
        ("Madrid to Bangalore", "Update customers table changing name of Madrid City to Bangalore.", "UPDATE customers SET City = 'Bangalore' WHERE City = 'Madrid';", "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE City = 'Bangalore';"),
        ("German to Bhutanese Customers", "Update all German customer to Bhutanese customers.", "UPDATE customers SET Country = 'Bhutan' WHERE Country = 'Germany';", "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE Country = 'Bhutan';"),
        ("Spanish to Thai Customers", "Convert all Spanish customer to Thai Customers.", "UPDATE customers SET Country = 'Thailand' WHERE Country = 'Spain';", "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE Country = 'Thailand';"),
        ("London to Kathmandu, Nepal", "Convert all customers of London to Kathmandu customers and also change the country.", "UPDATE customers SET City = 'Kathmandu', Country = 'Nepal' WHERE City = 'London';", "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE City = 'Kathmandu' AND Country = 'Nepal';"),
        ("G7 to SAARC Countries", "Convert customers from G7 Countries to SAARC Countries.", g7_to_saarc, "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE Country IN ('Afghanistan', 'Bangladesh', 'Bhutan', 'India', 'Maldives', 'Nepal', 'Pakistan', 'Sri Lanka');"),
        ...
        ("Delete Rome/Paris Postal 00 Customers", "DELETE all from customers having PostalCode containing 00 and from Rome or Paris.", "DELETE FROM customers WHERE PostalCode LIKE '%00%' AND City IN ('Rome', 'Paris');", "SELECT CustomerID, CustomerName, City, PostalCode, Country FROM customers;"),
        ("Delete Customers Not from Salzburg", "DELETE all from customers who are not from Salzburg.", delete_not_salzburg, "SELECT CustomerID, CustomerName, City, Country FROM customers;"),
        ("Delete July-born Employees", "DELETE all employees who are born in July.", delete_employee_month(7), "SELECT EmployeeID, LastName, FirstName, BirthDate FROM employees;"),
        ("Delete August-born Employees", "DELETE all employees who are born in August.", delete_employee_month(8), "SELECT EmployeeID, LastName, FirstName, BirthDate FROM employees;"),
        ("Delete Seafood and Beverage Products", "DELETE all product from seafood and beverages categories.", delete_product_categories, "SELECT ProductID, ProductName, CategoryID, Price FROM products;"),
        ("Supplier Count by Country", "Find name and count of suppliers from different countries.", "SELECT Country, COUNT(*) AS SupplierCount FROM suppliers GROUP BY Country;", None),
        ("Customer Count by City", "Find name and count of customers from different cities.", "SELECT City, COUNT(*) AS CustomerCount FROM customers GROUP BY City;", None),
        ("Orders by Employee", "Find number of orders made by employee along with EmployeeID.", "SELECT EmployeeID, COUNT(*) AS OrderCount FROM orders GROUP BY EmployeeID;", None),
    ]
...
""".strip()
    ...
    q5b = [
        ...
        ("Add Four Products", "Add 4 products in product table.", "INSERT INTO products (ProductName, SupplierID, CategoryID, Unit, Price) VALUES ('Nepal Tea', 1, 1, '20 tea bags', 12.50), ('Tokyo Noodles', 2, 5, '10 packets', 8.75), ('Cajun Spice Mix', 3, 3, '6 jars', 16.00), ('Nordic Cheese', 5, 4, '12 packs', 24.50);", "SELECT * FROM products WHERE ProductName IN ('Nepal Tea', 'Tokyo Noodles', 'Cajun Spice Mix', 'Nordic Cheese');"),
        ("Add Three Employees", "Add 3 new employees.", "INSERT INTO employees (LastName, FirstName, BirthDate, Notes) VALUES ('Shrestha', 'Anil', '1995-01-15', 'BSc in Computer Science.'), ('Karki', 'Mina', '1994-05-21', 'MBA in Operations.'), ('Gurung', 'Rita', '1996-11-10', 'BA in Management.');", "SELECT * FROM employees WHERE LastName IN ('Shrestha', 'Karki', 'Gurung');"),
        ("Mexico to Vietnam", "Update all customers from Mexico to Vietnam.", "UPDATE customers SET Country = 'Vietnam' WHERE Country = 'Mexico';", "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE Country = 'Vietnam';"),
        ("Paris to Delhi", "Update customers table changing name of Paris City to Delhi.", "UPDATE customers SET City = 'Delhi' WHERE City = 'Paris';", "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE City = 'Delhi';"),
        ("USA to Indian Customers", "Update all USA customer to Indian customers.", "UPDATE customers SET Country = 'India' WHERE Country = 'USA';", "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE Country = 'India';"),
        ("Brazilian to Cambodian Customers", "Convert all Brazilian customer to Cambodian Customers.", "UPDATE customers SET Country = 'Cambodia' WHERE Country = 'Brazil';", "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE Country = 'Cambodia';"),
        ("Madrid to Kathmandu, Nepal", "Convert all customers of Madrid to Kathmandu customers and also change the country.", "UPDATE customers SET City = 'Kathmandu', Country = 'Nepal' WHERE City = 'Madrid';", "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE City = 'Kathmandu' AND Country = 'Nepal';"),
        ("G7 to SAARC Countries", "Convert customers from G7 Countries to SAARC Countries.", g7_to_saarc, "SELECT CustomerID, CustomerName, City, Country FROM customers WHERE Country IN ('Afghanistan', 'Bangladesh', 'Bhutan', 'India', 'Maldives', 'Nepal', 'Pakistan', 'Sri Lanka');"),
        ("Delete Orders and Order Details", "DELETE all records from orders and orderdetails.", "DELETE FROM orderdetails;\nDELETE FROM orders;", "SELECT (SELECT COUNT(*) FROM orders) AS OrdersRemaining, (SELECT COUNT(*) FROM orderdetails) AS OrderDetailsRemaining;"),
        ("Delete Postal Code 31 Customers", "DELETE all from customers who have PostalCode containing 31.", "DELETE FROM customers WHERE PostalCode LIKE '%31%';", "SELECT COUNT(*) AS CustomersRemaining FROM customers;"),
        ("Delete Rome/Paris Postal 00 Customers", "DELETE all from customers having PostalCode containing 00 and from Rome or Paris.", "DELETE FROM customers WHERE PostalCode LIKE '%00%' AND City IN ('Rome', 'Paris');", "SELECT CustomerID, CustomerName, City, PostalCode, Country FROM customers;"),
        ...
        ("Delete July-born Employees", "DELETE all employees who are born in July.", delete_employee_month(7), "SELECT EmployeeID, LastName, FirstName, BirthDate FROM employees;"),
        ("Delete August-born Employees", "DELETE all employees who are born in August.", delete_employee_month(8), "SELECT EmployeeID, LastName, FirstName, BirthDate FROM employees;"),
        ("Delete Singapore and Japan Suppliers", "DELETE all suppliers from Singapore and Japan.", delete_suppliers_singapore_japan, "SELECT SupplierID, SupplierName, City, Country FROM suppliers;"),
        ("Supplier Count by Country", "Find name and count of suppliers from different countries.", "SELECT Country, COUNT(*) AS SupplierCount FROM suppliers GROUP BY Country;", None),
        ("Customer Count by City", "Find name and count of customers from different cities.", "SELECT City, COUNT(*) AS CustomerCount FROM customers GROUP BY City;", None),
        ("Orders by Employee", "Find number of orders made by employee along with EmployeeID.", "SELECT EmployeeID, COUNT(*) AS OrderCount FROM orders GROUP BY EmployeeID;", None),
    ]
...
""".strip()
    ...
    q6a = [
        ...
        ("Create cust View", "Create view cust by selecting only 4 attributes from customers.", "DROP VIEW IF EXISTS cust;\nCREATE VIEW cust AS\nSELECT CustomerID, CustomerName, City, Country\nFROM customers;\nSELECT * FROM cust;", None, None),
        ("Create productinfo View", "Join products with categories and suppliers to create productinfo with ProductID, ProductName, Unit, Price and all category/supplier attributes.", PRODUCTINFO_VIEW + ";\nSELECT * FROM productinfo;", None, None),
        ("Category Count from productinfo", "Find name and count of Categories from productinfo.", "SELECT CategoryName, COUNT(*) AS ProductCount FROM productinfo GROUP BY CategoryName;", None, PRODUCTINFO_VIEW),
        ("North American Supplier Products", "Find all products that have suppliers from North America in productinfo.", "SELECT ProductID, ProductName, SupplierName, SupplierCity, SupplierCountry, Price FROM productinfo WHERE SupplierCountry IN ('USA', 'Canada', 'Mexico');", None, PRODUCTINFO_VIEW),
        ("Products Below Average Price", "Find all products that have price lower than Average price from productinfo.", "SELECT ProductID, ProductName, Price FROM productinfo WHERE Price < (SELECT AVG(Price) FROM productinfo);", None, PRODUCTINFO_VIEW),
        ("Create orderinfo View", "Join orders with customers, employees and shippers to create a view as orderinfo.", ORDERINFO_VIEW + ";\nSELECT * FROM orderinfo;", None, None),
        ("United Package and Swift Shipping Orders", "List all order shipment provided by United Package and Swift Shipping from orderinfo.", "SELECT OrderID, OrderDate, CustomerName, ShipperID, ShipperName FROM orderinfo WHERE ShipperName IN ('United Package', 'Swift Shipping');", None, ORDERINFO_VIEW),
        ("Orders Processed by Janet and Steven", "List all order processed by employee Janet Leverling and Steven Buchanan from orderinfo.", "SELECT OrderID, OrderDate, CustomerName, EmployeeID, EmployeeName FROM orderinfo WHERE EmployeeName IN ('Janet Leverling', 'Steven Buchanan');", None, ORDERINFO_VIEW),
        ("Shipment Providers on 18 August 2023", "List all order shipment provider on date August 18, 2023 from orderinfo.", "SELECT OrderID, OrderDate, ShipperID, ShipperName FROM orderinfo WHERE OrderDate = '2023-08-18';", None, ORDERINFO_VIEW),
        ("Customer Countries on 19 August 2023", "List countries of customers that placed order on August 19, 2023 from orderinfo.", "SELECT DISTINCT CustomerCountry FROM orderinfo WHERE OrderDate = '2023-08-19';", None, ORDERINFO_VIEW),
    ]
    ...
    q6b = [
        ("UNION Customers and Employees", "Perform UNION of customers and employees relation using compatible attributes.", union_relation, None, None),
        ("UNION ALL Contact and Employee Names", "Perform UNION ALL of customers.ContactName and employees.FirstName or full name.", union_all_names, None, None),
        ("Berlin and Paris Customers", "List all customers from Berlin, Germany and Paris, France using UNION.", f"SELECT {CUSTOMER_COLS} FROM customers WHERE City = 'Berlin' AND Country = 'Germany'\nUNION\nSELECT {CUSTOMER_COLS} FROM customers WHERE City = 'Paris' AND Country = 'France';", None, None),
        ("USA Customers with Names Unlike UK", "List all customers of USA not having the same name as customers from UK.", f"SELECT {CUSTOMER_COLS}\nFROM customers AS usa\nWHERE usa.Country = 'USA'\n  AND NOT EXISTS (\n    SELECT 1 FROM customers AS uk\n    WHERE uk.Country = 'UK' AND uk.CustomerName = usa.CustomerName\n  );", None, None),
        ("Create cust View", "Create view cust by selecting only 4 attributes from customers.", "DROP VIEW IF EXISTS cust;\nCREATE VIEW cust AS\nSELECT CustomerID, CustomerName, City, Country\nFROM customers;\nSELECT * FROM cust;", None, None),
        ("Create productinfo View", "Join products with categories and suppliers to create productinfo with ProductID, ProductName, Unit, Price and all category/supplier attributes.", PRODUCTINFO_VIEW + ";\nSELECT * FROM productinfo;", None, None),
        ("Category Count from productinfo", "Find name and count of Categories from productinfo.", "SELECT CategoryName, COUNT(*) AS ProductCount FROM productinfo GROUP BY CategoryName;", None, PRODUCTINFO_VIEW),
        ("Asian Supplier Products", "Find all products that have suppliers from Asia in productinfo.", "SELECT ProductID, ProductName, SupplierName, SupplierCity, SupplierCountry, Price FROM productinfo WHERE SupplierCountry IN ('Japan', 'China', 'India', 'Nepal', 'Bangladesh', 'Bhutan', 'Pakistan', 'Sri Lanka', 'Maldives', 'Singapore', 'Thailand', 'Vietnam', 'Cambodia', 'South Korea');", None, PRODUCTINFO_VIEW),
        ("Products Below Average Price", "Find all products that have price lower than Average price from productinfo.", "SELECT ProductID, ProductName, Price FROM productinfo WHERE Price < (SELECT AVG(Price) FROM productinfo);", None, PRODUCTINFO_VIEW),
        ("Create orderinfo View", "Join orders with customers, employees and shippers to create a view as orderinfo.", ORDERINFO_VIEW + ";\nSELECT * FROM orderinfo;", None, None),
        ("Speedy and Global Shipping Orders", "List all order shipment provided by Speedy Express and Global Express from orderinfo.", "SELECT OrderID, OrderDate, CustomerName, ShipperID, ShipperName FROM orderinfo WHERE ShipperName IN ('Speedy Express', 'Global Express');", None, ORDERINFO_VIEW),
        ("Orders Processed by Nancy and Michael", "List all order processed by employee Nancy Davolio and Michael Suyama from orderinfo.", "SELECT OrderID, OrderDate, CustomerName, EmployeeID, EmployeeName FROM orderinfo WHERE EmployeeName IN ('Nancy Davolio', 'Michael Suyama');", None, ORDERINFO_VIEW),
        ("Shipment Providers on 19 August 2023", "List all order shipment provider on date August 19, 2023 from orderinfo.", "SELECT OrderID, OrderDate, ShipperID, ShipperName FROM orderinfo WHERE OrderDate = '2023-08-19';", None, ORDERINFO_VIEW),
        ("Customer Countries on 17 August 2023", "List countries of customers that placed order on August 17, 2023 from orderinfo.", "SELECT DISTINCT CustomerCountry FROM orderinfo WHERE OrderDate = '2023-08-17';", None, ORDERINFO_VIEW),
    ]
